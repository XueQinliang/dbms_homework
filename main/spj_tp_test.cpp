...
template<class T> class spj_condition_obj { //一个元素的类
	public:
		int var_con; //0:是一个列；1:是一个常数
		Table *table; //表指针（适用于0）
		string var_name; //列名（适用于0）
		T value; //常数（适用于1）
};

template<class T> class spj_condition { //一个where子句的类
	public:
		spj_condition_obj<T> a, b; //元素a, b
		string op; //运算符，包括：=，<>，<，<=，>，>=
};
...
vector<int> *spj_select(Table *t, vector<pair<string, void *> > exprs, string conj) { //返回的是item的数组
...
vector<vector<pair<string, void *> > *> *spj_project(Table *t, vector<string> *var_names,
        vector<int> *pk) { //string表示类型, void *指向数据
...
vector<vector<pair<string, void *> > > spj(vector<Table *> &tables, vector<pair<Table *, string> > &table_var_names,
	...
	map<Table *, vector<int> *> pks; //先每列自己select
