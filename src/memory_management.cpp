...
public:
    ...
    int num_pages;
    int num_items;
    vector<pair<int, int>> to_write;
    string table_name;
    __gnu_cxx::hash_map<int, Page *> idx2page;
    map<Page *, int> page2idx;
    __gnu_cxx::hash_map<string, pair<int, int>> item_dict;
    static vector<string> types;
    static vector<string> constraints;
    static regex redundant;
    vector<string> attributes;
    __gnu_cxx::hash_map<string, tuple<string, int, int, int>> attr_dict;
...
Table::Table(string table_name, vector<vector<string>> vars_with_constraints) : table_name(table_name) {
    ...
    string data_path = table_name + ".td";
    string meta_path = table_name + ".tm";
...
void Table::del_item_with_pk(string pk) {
    ...
    else to_write.emplace_back(item_dict[pk]);
}
...
void Table::write_seg_in_mem() {
    ...
    for (int i = 0; i < num_pages; ++i) {
        if (idx2page.find(i) != idx2page.end()) {
